...
Master input:
    packaging/icons/openxvt-icon.png       1024x1024 RGBA PNG

Derived outputs (committed to the repository):
    cmake/windows/openxvt.ico                  Windows executable icon
    cmake/macos/OpenXvT.icns                   macOS bundle icon
    src/xvt_app/window_icon.h              embedded BMP passed to Aeron for
                                           SDL_SetWindowIcon
...
MASTER = os.path.join(REPO_ROOT, "packaging", "icons", "openxvt-icon.png")
ICO_OUT = os.path.join(REPO_ROOT, "cmake", "windows", "openxvt.ico")
ICNS_OUT = os.path.join(REPO_ROOT, "cmake", "macos", "OpenXvT.icns")
HEADER_OUT = os.path.join(REPO_ROOT, "src", "xvt_app", "window_icon.h")
...
def emit_header(bmp):
    lines = [
        ...
        "",
        f"static const unsigned char xvt_window_icon_bmp[{len(bmp)}] = {{",
    ]
