...
deployment_target="${XVT_MACOS_DEPLOYMENT_TARGET:-13.0}"
architecture="${XVT_MACOS_ARCHITECTURE:-$(uname -m)}"
application_version="${XVT_VERSION:-0.0.0-dev}"
build_version="${XVT_MACOS_BUILD_VERSION:-1}"
build_type="${XVT_BUILD_TYPE:-Release}"
bundle_identifier="${XVT_MACOS_BUNDLE_IDENTIFIER:-org.openxvt.openxvt}"
sign_identity="${XVT_MACOS_SIGN_IDENTITY:--}"
notary_profile="${XVT_MACOS_NOTARY_PROFILE:-}"
notary_apple_id="${XVT_MACOS_NOTARY_APPLE_ID:-}"
notary_team_id="${XVT_MACOS_NOTARY_TEAM_ID:-}"
notary_password="${XVT_MACOS_NOTARY_PASSWORD:-}"

# The Debug variant packages an OpenXvT.dSYM
# debug-symbol bundle beside the application. Dependencies are always built in
# Release mode; only OpenXvT changes configuration, so both variants share the
...
work_root="${XVT_MACOS_BUILD_ROOT:-${repo_root}/build/cache/macos-${architecture}}"
artifact_dir="${XVT_MACOS_ARTIFACT_DIR:-${repo_root}/build/artifacts}"
source_root="${work_root}/sources"
dependency_prefix="${work_root}/dependencies"
tool_prefix="${work_root}/tools"
build_root="${work_root}/build"
shadercross_executable="${XVT_MACOS_SHADERCROSS_EXECUTABLE:-${build_root}/shadercross/shadercross}"
stage_root="${work_root}/stage-${build_variant}"
application="${stage_root}/OpenXvT.app"
dmg="${artifact_dir}/openxvt-${application_version}-macos-${architecture}-${build_variant}.dmg"
dmg_volume_name="OpenXvT"
...
build_application() {
    ...
    export PKG_CONFIG_LIBDIR="${dependency_prefix}/lib/pkgconfig"

    cmake -S "${repo_root}" \
        -B "${build_root}/xvt-${build_variant}" \
        -G Ninja \
        ...
        "-DXVT_ENABLE_DEBUG_UI=${debug_ui}" \
        "-DXVT_MACOS_BUILD_VERSION=${build_version}" \
        "-DXVT_MACOS_BUNDLE_IDENTIFIER=${bundle_identifier}" \
        "-DXVT_VERSION=${application_version}"
    cmake --build "${build_root}/xvt-${build_variant}" --target xvt \
        --parallel "${jobs}"

    cmake -E remove_directory "${stage_root}"
    cmake --install "${build_root}/xvt-${build_variant}" --component Runtime
    ...
    if [[ "${build_type}" == Debug ]]; then
        dsym_bundle="${stage_root}/OpenXvT.dSYM"
        dsymutil "${application}/Contents/MacOS/OpenXvT" -o "${dsym_bundle}"
    fi
}
...
main() {
    ...
    case "${architecture}" in
        arm64|x86_64) ;;
        *) echo "XVT_MACOS_ARCHITECTURE must be arm64 or x86_64" >&2; exit 2 ;;
    esac
    ...
    build_dependencies
    if [[ -z "${XVT_MACOS_SHADERCROSS_EXECUTABLE:-}" ]]; then
        build_shadercross
    ...
    test -x "${application}/Contents/MacOS/OpenXvT"
    ...
    test -f "${application}/Contents/Resources/shaders/hyperspace_streak.frag.msl"
    ...
    if [[ "${build_type}" == Debug ]]; then
        application_uuid="$(dwarfdump --uuid \
            "${application}/Contents/MacOS/OpenXvT" | awk 'NR == 1 {print $2}')"
        dsym_uuid="$(dwarfdump --uuid "${dsym_bundle}" \
            | awk 'NR == 1 {print $2}')"
        if [[ -z "${application_uuid}" ||
                "${application_uuid}" != "${dsym_uuid}" ]]; then
            echo "OpenXvT.dSYM does not match the built executable" >&2
            exit 1
        fi
    fi
    validate_bundle_paths
