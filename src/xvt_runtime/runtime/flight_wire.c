...
uint64_t XvtWire_Get64(const XvtWireU64 value) { return XvtFlightWire_Read(value, sizeof(XvtWireU64)); }
...
int XvtFlightWire_ValidTick(uint32_t tick) {
	return tick && tick <= INT32_MAX - 1u && tick % XVT_NETWORK_STEP_TICKS == 0;
}
